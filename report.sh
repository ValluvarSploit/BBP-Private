...
YELLOW='\033[0;33m'
subd="$1"
mkdir aquatone-output/parsedjson
mkdir reports
...
cat ./aquatone-output/aquatone_session.json | jq --arg v "$subd" -r '.pages[$v].headers[] | keys[] as $k | "\($k), \(.[$k])"' | grep -v "decreasesSecurity\|increasesSecurity" >> ./aquatone-output/parsedjson/$subdomain.headers
dirsearchfile=$(ls dirsearch/reports/$subdomain/ | grep -v old)

touch ./reports/$subdomain.html
echo '<html><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">' >> ./reports/$subdomain.html
echo "<head>" >> ./reports/$subdomain.html
...
<script type="text/javascript" charset="utf8" src="https://cdn.datatables.net/1.10.19/js/jquery.dataTables.js"></script><script type="text/javascript" charset="utf8" src="https://cdn.datatables.net/1.10.19/js/dataTables.material.min.js"></script>'>> ./reports/$subdomain.html
...
} );</script></head>'>> ./reports/$subdomain.html

echo '<body class="dark"><header class="site-header">
<div class="site-title"><p>' >> ./reports/$subdomain.html
...
</header>" >> ./reports/$subdomain.html
echo '<div id="wrapper"><div id="container">'  >> ./reports/$subdomain.html
echo "<h1 class=\"post-title\" itemprop=\"name headline\">Recon Report for <a href=\"https://$subdomain\">$subdomain</a></h1>" >> ./reports/$subdomain.html
echo "<p class=\"blog-description\">Generated by LazyRecon on $(date) </p>" >> ./reports/$subdomain.html
...
<h2>Content Discovery</h2>' >> ./reports/$subdomain.html



  echo "<table id='myTable' class='stripe'>" >> ./reports/$subdomain.html
  echo "<thead><tr>
 <th>Status Code</th>
 <th>Content-Length</th>
 <th>Url</th>
 </tr></thead><tbody>" >> ./reports/$subdomain.html
  ...
  path=${url#*[0-9]/}
 echo "<tr>" >> ./reports/$subdomain.html
 if [[ "$status_code" == *20[012345678]* ]]; then
    echo "<td class='status jackpot'>$status_code</td><td class='status jackpot'>$size</td><td><a class='status jackpot' href='$url'>/$path</a></td>" >> ./reports/$subdomain.html
  elif [[ "$status_code" == *30[012345678]* ]]; then
    echo "<td class='status redirect'>$status_code</td><td class='status redirect'>$size</td><td><a class='status redirect' href='$url'>/$path</a></td>" >> ./reports/$subdomain.html
  elif [[ "$status_code" == *40[012345678]* ]]; then
    echo "<td class='status fourhundred'>$status_code</td><td class='status fourhundred'>$size</td><td><a class='status fourhundred' href='$url'>/$path</a></td>" >> ./reports/$subdomain.html
  elif [[ "$status_code" == *50[012345678]* ]]; then
    echo "<td class='status fivehundred'>$status_code</td><td class='status fivehundred'>$size</td><td><a class='status fivehundred' href='$url'>/$path</a></td>" >> ./reports/$subdomain.html
  else
     echo "<td class='status weird'>$status_code</td><td class='status weird'>$size</td><td><a class='status weird' href='$url'>/$path</a></td>" >> ./reports/$subdomain.html
  fi
 echo "</tr>">> ./reports/$subdomain.html
done

  echo "</tbody></table></div>" >> ./reports/$subdomain.html
...
<div class="post-content clearfix" itemprop="articleBody">
<h2>Screenshots</h2>
<pre style="max-height: 340px;overflow-y: scroll">' >> ./reports/$subdomain.html
echo '<div class="row">
<div class="column">
Port 80' >> ./reports/$subdomain.html
scpath=$(echo "$subdomain" | sed 's/\./_/g')
httpsc=$(ls ./aquatone-output/screenshots/http__$scpath*  2>/dev/null)
echo "<a href=\"../../../$httpsc\"><img/src=\"../../../$httpsc\"></a> " >> ./reports/$subdomain.html
echo '</div>
  <div class="column">
Port 443' >> ./reports/$subdomain.html
httpssc=$(ls ./aquatone-output/screenshots/https__$scpath*  2>/dev/null)
echo "<a href=\"../../../$httpssc\"><img/src=\"../../../$httpssc\"></a>" >> ./reports/$subdomain.html
echo "</div></div></pre>" >> ./reports/$subdomain.html
#echo "<h2>Dig Info</h2><pre>$(dig $subdomain)</pre>" >> ./$domain/$foldername/reports/$subdomain.html
echo "<h2>Host Info</h2><pre>$(host $subdomain)</pre>" >> ./reports/$subdomain.html
echo "<h2>Response Headers</h2><pre>" >> ./reports/$subdomain.html




cat ./aquatone-output/parsedjson/$subdomain.headers | while read ln;do
check=$(echo "$ln" | awk '{print $1}')

[ "$check" = "name," ] && echo -n "$ln : " | sed 's/name, //g' >> ./reports/$subdomain.html
[ "$check" = "value," ] && echo " $ln" | sed 's/value, //g' >> ./reports/$subdomain.html

done



echo "</pre>" >> ./reports/$subdomain.html
...
</div></div></body></html>" >> ./reports/$subdomain.html
